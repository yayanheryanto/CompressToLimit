...
export function UploadZone({ onFileSelected }: UploadZoneProps) {
  ...
  const processFile = useCallback(
    (file: File) => {
      ...
      if (file.size > MAX_BYTES) {
        toast.error('File exceeds 50 MB limit. Please choose a smaller file.');
        return;
      }
  ...
  return (
    ...
    >
      ...
      <h3 className="mb-1 text-base font-semibold text-slate-800">Drop your file here</h3>
      <p className="mb-5 text-sm text-slate-400">or click to browse from your computer</p>
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        ...
        <span className="self-center text-[11px] text-slate-400">· Max 50 MB</span>
      </div>
    </motion.div>
  );
}
